...
import {timezonesDataAccessPoint} from "@/timezones/useTimezones.ts";

const meta: Meta<typeof Timezones> = {
  title: 'App/Timezones',
  component: Timezones,
}
export default meta;
...
export const Primary: Story = {
  parameters: {
    msw: {
      handlers: [
        rest.get(timezonesDataAccessPoint, (_, res, ctx) => {
          // console.log(req.url);
          return res(
...
export const NetworkTimeout: Story = {
  parameters: {
    msw: {
      handlers: [
        rest.get(timezonesDataAccessPoint, (_, res) => {
  ...
  parameters: {
    msw: {
      handlers: [
        rest.get(timezonesDataAccessPoint, (_, res, ctx) => {
          return res(
            ctx.xml('<xml> This is a syntax error from server </xml>')
          )
        }),
      ]
    }
